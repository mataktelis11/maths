...
ab h metathesh einai tautotikh kai stin periptwsh pou auto isxuei h 'Iperm' tha ginei 'True'
alliws 'False'
...
kyklwn xwris tous kyklous me mhkos 1 

Meta 'sarwnoume' mia fora to array 'AllCycles' gia na doume an yparxoun kykloi mhkous 1.
An ypaxoun tote h boolean 'hasone'ginetai true alliws false

An h metathesi einai tautotiki h exei mono ena stoixeio ( m=(1) ) h den exei kyklous mhkous 1
...
Telos kaleitai h synartisi 'countercycles':
    ...
    H synartisi gemiszei to array 'CounterCycles' me tis antimetatheseis ths metatheshs
...
def printstrangecycles():
    ...
    print("Metathesi san ginomeno 3enwn kyklwn(xwris kyklous mhkous 1):")
    print("m =",end=" ")
...
if(checkpermutation(permutation)==True):
    ...
    if(Iperm==False) and (len(AllCycles)>1) and (hasone==True):
        printallcycles()
        printstrangecycles()
    else:
        printallcycles()   
